...
if __name__ == '__main__':
    ...
    embedding_method = ['MDS', 'poincare', 'struc2vec']
    ...
    embedding_file = 'structural_neighborhood/outf_nodes_space_relation_' + dataset_name + '_' + embedding_name + '.txt'


    # load structural neighborhood
    new_edge_index, edge_relation = utils.load_structure_neighborhood(embedding_file, data.x.shape[0])
    ...
    net = GeomGCN(data, edge_relation, num_features, num_hidden, num_classes, dropout, layer_num=2, device=device)
    net = net.to(device)
